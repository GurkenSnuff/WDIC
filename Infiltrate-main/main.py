...
def main_menu():
    ...
    while game_state == 1:
        ...
        play_button.update(screen, art.BLACK)
        settings_button.update(screen, art.BLACK)
        tutorial_button.update(screen, art.BLACK)
        quit_button.update(screen, art.BLACK)
        if are_you_sure:
            are_you_sure_button.update(screen, art.BLACK)
            yes_button.update(screen, art.BLACK)
            no_button.update(screen, art.BLACK)
...
def settings_menu():
    ...
    while game_state == 2:
        ...
        art.draw_menu_border(w, h, screen)
        asteroid_effect(ast_list)
        back_button.update(screen, art.BLACK)
        reset_button.update(screen, art.BLACK)
        if sound:
...
def draw_display_text(grid_w, unit):
    ...
    if unit.team is None:
        ...
        if unit.static:
            ...
            cancel_build_button.update(screen, art.BLACK)
        else:
            if unit.can_attack:
...
def tutorial():
    ...
    while game_state == 4:
        ...
        back_button.update(screen, art.BLACK)
        next_button.update(screen, art.BLACK)
        turn_button.update(screen, art.BLACK)
                ...
                if grid_space.get_rect(x=(adj_x), y=(adj_y)).collidepoint(event.pos):
                    ...
                    if event.button == 5:
                        if grid_scroll < grid_height - grid_width:
                            grid_scroll += 1
                            scroller.update_y(grid_scroll)
                    dest_cell = gridClass.find_grid_coords(event.pos[0], event.pos[1], grid_w, grid_width, grid_scroll, adj_y, adj_x)
                    ...
                    if selected_unit.static:
                        if selected_unit.produce is not None and type(selected_unit.produce) == tuple:
                            ...
                            for num, option in enumerate(tp_options, 1):
                                ...
                                current_button.update(screen, art.GREEN)
                        ...
                        if selected_unit.steps_remaining == selected_unit.speed:
                            move_button.update(screen, art.GREEN)
                        elif selected_unit.steps_remaining > 0:
                            move_button.update(screen, art.BLACK)
                        else:
                            selected_unit.activities[0] = 1
                            move_button.update(screen, art.RED)
                        if selected_unit.has_attacked or not selected_unit.can_attack:
                            selected_unit.activities[1] = 1
                            attack_button.update(screen, art.RED)
                        else:
                            attack_button.update(screen, art.GREEN)
                        if selected_unit.special_used or not selected_unit.special_condition:
                            selected_unit.activities[2] = 1
                            special_button.update(screen, art.RED)
                        else:
                            special_button.update(screen, art.GREEN)
...
def game():
    ...
    while game_state == 3:
                ...
                if grid_space.get_rect(x=(adj_x), y=(adj_y)).collidepoint(event.pos) and not game_over:
                    if event.button == 3:
                        ...
                        if build_request is not None:
                            ...
                            request_dest = gridClass.find_grid_coords(event.pos[0], event.pos[1], grid_w, grid_width, grid_scroll, adj_y, adj_x)
        ...
        for unit in item_list:
            if unit.team != None:
                unit.ShowLifeBar(screen, grid_w, grid_width, grid_height, grid_scroll, adj_y, adj_x)
